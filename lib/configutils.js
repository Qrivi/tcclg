...
async function verify (config) {
  ...
  let setup = [
    [ chalk.bold('Project name'), chalk.bold('Last build'), chalk.bold('Active branch'), '' ]
  ]
    ...
    config.projects.forEach((project, index) => {
      if (!project.include) {
        ...
        if (!project.build) { // getBuildNumber returns false if no number is found on given url.
          ...
          setup.push([ project.name, project.build + buildFetched, project.branch, chalk.bgGreen.bold(' Including ') ])
        }
      } else {
        success = false
