...
async function verify (config) {
  ...
  if (config.teamcity && config.teamcity.url) {
    ...
    if (requestUtils.canConnect(config.teamcity.url, '/httpAuth/app/rest', config.teamcity.username, config.teamcity.password)) {
      ...
      success = false
      logUtils.error('✘ Connecting to TeamCity has failed.', true, true)
    }
  } else {
    success = false
  ...
  if (config.jira && config.jira.length && Array.isArray(config.jira)) {
      ...
      if (jira.url && jira.projectKey) {
        if (!jira.include) {
          ...
          if (requestUtils.canConnect(jira.url, '/rest/api/2/myself', jira.username, jira.password)) {
            ...
            success = false
            logUtils.error(`✘ Connecting to Jira for ${jira.projectKey} issues has failed.`, true, true)
          }
        }
      } else {
        success = false
    ...
    logUtils.error('✘ There are no valid Jira configurations defined.', true)
  }
  ...
  let setup = [
    [ chalk.bold('Project name'), chalk.bold('Last build'), chalk.bold('Build branch'), chalk.bold('Active branch'), '' ]
  ]
  ...
  if (config.projects && config.projects.length && Array.isArray(config.projects)) {
    config.projects.forEach((project, index) => {
      if (!project.include) {
        setup.push([ project.name, 'n/a', 'n/a', 'n/a', chalk.bgRed.bold(' Ignoring ') ])
        ...
        if (!project.build) { // getBuildNumber returns false if no number is found on given url.
          success = false
          logUtils.error(`✘ Could not find a build number for ${project.name}.`, true)
        } else {
          project.buildBranch = project.buildBranch || project.branch;
          setup.push([ project.name, project.build + buildFetched, project.buildBranch, project.branch, chalk.bgGreen.bold(' Including ') ])
        }
      } else {
        success = false
        logUtils.error(`✘ Project configuration ${index} is missing name, branch and/or build (endpoint).`, true)
      }
    })
  } else {
    success = false
