...
async function verify( config ) {
    ...
    if( config.teamcity && config.teamcity.url ) {
        ...
        if( requestUtils.canConnect( config.teamcity.url, '/httpAuth/app/rest', config.teamcity.username, config.teamcity.password ) ) {
            ...
            success = false;
            logUtils.error( '⚠️ Connecting to TeamCity failed.', true, true );
        }
    } else {
        success = false;
        logUtils.error( '⚠️ TeamCity configuration is missing the TeamCity URL.', true, true );
    }
    ...
    if( config.jira && config.jira.length && Array.isArray( config.jira ) ) {
        config.jira.forEach( ( jira, index ) => {
            ...
            if( jira.url && jira.projectKey ) {
                if( !jira.include ) {
                    logUtils.warn( `⚠️ ${jira.projectKey} issues will not be included.`, true, true );
                } else {
                    ...
                    if( requestUtils.canConnect( jira.url, '/rest/api/2/myself', jira.username, jira.password ) ) {
                        ...
                        success = false;
                    logUtils.log( `⚠️ Connecting to Jira for ${jira.projectKey} issues failed.`, true, true );
                    }
                }
            } else {
                success = false;
                logUtils.error( `⚠️ Jira configuration #${index} is missing the Jira URL.`, true, true );
            }
        } );
    } else {
    ...
    if( config.projects && config.projects.length && Array.isArray( config.projects ) ) {
        config.projects.forEach( ( project, index ) => {
            if( !project.include ) {
                ...
                success = false;
                logUtils.error( `⚠️ Project configuration ${index} is missing name, branch and/or version endpoint.`, true );
            }
        } );
    } else {
        success = false;
        logUtils.error( '⚠️ There are no valid project configurations defined.', true );
    }
