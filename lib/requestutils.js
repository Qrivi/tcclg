...
function getChanges (project, teamcity) {
  ...
  try {
    ...
    if (res.body.toString().includes('Cannot find build or build id for locator')) {
      ...
      logUtils.error(`✘ Build ${project.build} does not exist or was not built from ${project.branch}.`, true)
      return {
        name: project.name,
    ...
    if (!builds.length) {
      logUtils.warn(`There have been no new builds since ${project.build} from ${project.branch}.`, true)
      return {
        name: project.name,
        status: 'no-builds',
        currentBuild: project.build,
    ...
    logUtils.log(`There are changes since ${project.build} from ${project.branch}.`, true)
    return {
      name: project.name,
      status: 'ok',
      currentBuild: project.build,
