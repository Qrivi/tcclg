...
function getChanges (project, teamcity) {
  ...
  try {
    const url = `${teamcity.url.replace(/\/$/, '')}/httpAuth/app/rest/builds?locator=count:100,buildType:${project.buildType},branch:${project.branch},sinceBuild:(buildType:${project.buildType},branch:default:any,number:${project.build})&fields=build:(number,branchName,finishDate,changes(change(username,comment)))`
    ...
    try {
      logUtils.traceTeamCity(url, JSON.parse(res.body.toString()))
    } catch (err) {
      const body = res.body.toString()
      logUtils.traceTeamCity(url, body)

      if (body.includes('No build types found for locator')) {
        logUtils.error(`✘ Build type ${project.buildType} does not exist.`, true)
        return {
          name: project.name,
          status: 'buildtype-not-found',
          currentBuild: project.build,
          branch: project.branch
        }
      }

      if (body.includes('Cannot find build or build id for locator')) {
        logUtils.error(`✘ Build ${project.build} does not exist.`, true)
        return {
          name: project.name,
    ...
    const builds = JSON.parse(res.body.toString()).build
    if (!builds.length) {
    ...
    logUtils.error(err)
    logUtils.error(`TeamCity returned an unexpected response.`, true)
    return {
      name: project.name,
      status: 'error',
      currentBuild: project.build
...
function getIssueDetails (key, jira) {
  ...
  try {
    ...
    logUtils.traceJira(url, json, key)
